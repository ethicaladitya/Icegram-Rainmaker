		...
		function execute_shortcode($atts = array()){
			...
			if($rm_form_settings['type'] == 'custom'){
				...
				foreach ($form_type_data as  $key => $field) {
                    ...
                    }else if(in_array($key, array('email', 'name','subject','message')) && $is_email_present && !$is_name_present && (!$is_subject_present && !$is_message_present)) {
	                    $additional_class = 'rm_form_el_two_third';
	                }else if(in_array($key, array('email', 'name','subject','message')) && $is_name_present && !$is_subject_present && !$is_message_present) {
			...
			if(!empty($form_html)){
				...
				$html .= '<div id="' .$rm_form_id .'" class="rm_form_container rainmaker_form '. $rm_form_settings['form_style'] . ' ' .$form_layout_class.'" data-type="rm_'. $rm_form_settings['type'] .'" data-form-id="'. $atts['id'] .'">';
